...
<article id="respond">
     ...
     <div class="comment-login-option">Logged in as <a href="<?php echo get_option( 'siteurl' ); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url( get_permalink() ); ?>" title="Log out of this account">Log out &raquo;</a></div>
          ...
          <!--BEGIN #form-section-author-->
               <input name="author" id="author" type="text" value="<?php echo $current_user->user_nicename; ?>" tabindex="1" <?php if ( $req ) echo "aria-required='true'"; ?> class="textbox reply_name" placeholder="Name"/>
          <!--END #form-section-author-->
     

          <!--BEGIN #form-section-email-->
               <input name="email" id="email" type="email" value="<?php echo $current_user->user_email; ?>" tabindex="2" <?php if ( $req ) echo "aria-required='true'"; ?> class="textbox reply_email" placeholder="Email"/>
          <!--END #form-section-email-->
           
          <?php else : // If user isn't logged-in, ask them for their details ?>
        
          <!--BEGIN #form-section-author-->
               <input name="author" id="author" type="text" value="<?php echo $comment_author; ?>" tabindex="1" <?php if ( $req ) echo "aria-required='true'"; ?> class="textbox reply_name" placeholder="Name" />
          <!--END #form-section-author-->
     
          <!--BEGIN #form-section-email-->
               <input name="email" id="email" type="email" value="<?php echo $comment_author_email; ?>" tabindex="2" <?php if ( $req ) echo "aria-required='true'"; ?> class="textbox reply_email" placeholder="Email"/>
          <!--END #form-section-email-->

          <?php endif; // if ( is_user_logged_in() ) ?>
          
     <!--BEGIN #form-section-comment-->
               <textarea name="comment" id="comment" type="text" tabindex="4" class="reply_comment"placeholder="Comment"></textarea>
